...
public class AllRestauranteProductoMDB implements MessageListener, ExceptionListener {
	
	public final static int TIME_OUT = 5;
	private final static String APP = "appJPFP";
	
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicAllRestauranteProducto";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQAllRestauranteProductoLocal";
	...
	private ArrayList<OrdenRestaurante> answer = new ArrayList<OrdenRestaurante>();
	
	public AllRestauranteProductoMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
...
//		id = new String(md.digest(id.getBytes()));
		...
		ArrayList<OrdenRestaurante> res = answer;
        return res;
	}
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("RestauranteProducto.general.app1", APP, payload, status, id);
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ArrayList<OrdenRestaurante> RestauranteProducto = dtm.getLocalRestauranteProducto();					
					String payload = mapper.writeValueAsString(RestauranteProducto);
					Topic t = new RMQDestination("", "RestauranteProducto.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					OrdenRestaurante v = mapper.readValue(ex.getPayload(), OrdenRestaurante.class);
					answer.add(v);
				}
			}
