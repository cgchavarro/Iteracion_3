...
public class AllRestaurantesMDB implements MessageListener, ExceptionListener  {
	...
	private ArrayList<Restaurante> answer = new ArrayList<Restaurante>();
...
//		id = new String(md.digest(id.getBytes()));
		...
		ArrayList<Restaurante> res= answer;
        return res;
	}
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("Restaurantes.general.appJPFP", APP, payload, status, id);
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ArrayList<Restaurante> Restaurantes = dtm.getLocalRestaurantes();
					String payload = mapper.writeValueAsString(Restaurantes);
					Topic t = new RMQDestination("", "Restaurantes.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					Restaurante v = mapper.readValue(ex.getPayload(), Restaurante.class);
					answer.add(v);
				}
			}
